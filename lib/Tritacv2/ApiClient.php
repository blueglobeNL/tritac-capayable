...
class Tritacv2_ApiClient
{
     ...
     */
    public function __construct(Tritacv2_Configuration $config = null)
    {
        $configClass = new Tritacv2_Configuration();
        if ($config === null) {
            $config = $configClass->getDefaultConfiguration();
        }

        $this->config       = $config;
        $this->serializer   = new Tritacv2_ObjectSerializer();
        $date               = date('Y-m-d');
    ...
    {
        if($this->logging) {
            Mage::log('callApi resourcePath', null, $this->logfile);
            Mage::log($resourcePath, null, $this->logfile);
            Mage::log('callApi method', null, $this->logfile);
            Mage::log($method, null, $this->logfile);
            Mage::log('callApi queryParams', null, $this->logfile);
            Mage::log($queryParams, null, $this->logfile);
            Mage::log('callApi postData', null, $this->logfile);
            Mage::log($postData, null, $this->logfile);
            Mage::log('callApi headerParams', null, $this->logfile);
            Mage::log($headerParams, null, $this->logfile);
            Mage::log('callApi', null, $this->logfile);
        }
        ...
        if ($postData and in_array('Content-Type: application/x-www-form-urlencoded', $headers, true)) {
            ...
            // okay, let us take this one step at the time!
            $postData   = $objSer->sanitizeForSerialization($postData);
            // so please, what does it look like now???
            if($this->logging) {
                Mage::log('callApi postData after sanitizeForSerialization', null, $this->logfile);
                Mage::log($postData, null, $this->logfile);
            }
        ...
        if($this->logging) {
            Mage::log('callApi url ', null, $this->logfile);
            Mage::log($url, null, $this->logfile);
        }
        ...
        if ($method === self::$POST) {
            ...
            throw new Tritacv2_ApiException('Method ' . $method . ' is not recognized.');
        }
        curl_setopt($curl, CURLOPT_URL, $url);

        //Mage::log('callApi userAgent ', null, $this->logfile);
        $userAgent = $this->config->getUserAgent();

        // Set user agent
        curl_setopt($curl, CURLOPT_USERAGENT, $userAgent);//$this->config->getUserAgent());
        ...
        // Sol, it looks like this is the only error that is really of any use :
        if($this->logging) {
            Mage::log('callApi response ', null, $this->logfile);
        ...
        if($this->logging) {
            Mage::log('callApi response_info ', null, $this->logfile);
            Mage::log($response_info, null, $this->logfile);
        }
        ...
        if ($response_info['http_code'] === 0) {
            ...
            $exception = new Tritacv2_ApiException($error_message, 0, null, null);
            ...
            throw new Tritacv2_ApiException(
